...
def preprocess_pipeline(folder_path,rpath_T1,rpath_T2,rpath_PET,rpath_out_folder,idx,password,temp_folder,meta=None):  
    ...
    try:
        ...
        if meta is None:
            nib.save(img, os.path.join(folder_path,os.path.join(rpath_out_folder,'PET_%d_raw_avg_1mm.nii'%(idx))))
            nib.save(img, os.path.join(folder_path,os.path.join(rpath_out_folder,'T2_%d_raw_1mm.nii'%(idx))))
            nib.save(img, os.path.join(folder_path,os.path.join(rpath_out_folder,'T1_%d_raw_1mm.nii'%(idx))))
        else:
            nib.save(img, os.path.join(folder_path,os.path.join(rpath_out_folder,'PET_%d_raw_avg_1mm_%s_%s_%s_%s.nii'%(idx,meta[0],meta[1],meta[2],meta[3]))))
            nib.save(img, os.path.join(folder_path,os.path.join(rpath_out_folder,'T2_%d_raw_1mm_%s_%s_%s_%s.nii'%(idx,meta[0],meta[1],meta[2],meta[3]))))
            nib.save(img, os.path.join(folder_path,os.path.join(rpath_out_folder,'T1_%d_raw_1mm_%s_%s_%s_%s.nii'%(idx,meta[0],meta[1],meta[2],meta[3]))))
